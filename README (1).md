...
| Neural Network | 0.856 | 0.672 |

**Confusion matrices** (see notebook) show the neural network correctly identified more true
`>50K` cases than logistic regression and had fewer false negatives, at the cost of slightly
more false positives. (Exact counts may vary slightly by ~1% between runs due to random
initialization in the neural network — see the notebook for the final run's exact figures.)
...
small (roughly 0.5–1.5 percentage points). Accuracy alone would understate the practical
